use crate::{
    ...
    db::{self, CompletedStatus, check_init_completed, completed_init},
    service,
    service::model::try_save_model_info,
};
use clap::Args;
...
pub async fn init_local_registry(args: InitArgs) {
    ...
    match try_save_model_info(Arc::clone(&conn), client, remote.clone()).await {
        Ok(_) => {
