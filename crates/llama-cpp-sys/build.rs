...
fn main() {
    let cargo_manifest_dir = PathBuf::from(
        env::var("CARGO_MANIFEST_DIR").expect("Couldn't get cargo manifest dir from env"),
    );
    // 将 bindgen 生成代码文件放置到该目录下
    let binding_rs_out_dir = cargo_manifest_dir.join("llama");
    // 编译产物所在的路径
    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("Couldn't get out dir form env"));
    // llama.cpp 源码路径
    let llama_src_dir = cargo_manifest_dir.join("llama.cpp");
    ...
    bindings
        ...
        .expect("Failed to change binding's config when set out file");

    // 获取目标三元组，针对不同的操作系统做不同的配置
    let target = TargetTriple::parse_from_env().expect("Failed to get target triple from env");
...
#[cfg(feature = "openmp")]
fn cargo_rustc_link_openmp_lib(target: &TargetTriple) {
    if target.is_gnu() {
...
#[cfg(any(feature = "cuda", feature = "cuda-no-vmm"))]
fn cargo_rustc_link_cuda_lib(target: &TargetTriple) {
    // 寻找 cuda 安装的路径
...
/// 设置 rustc 链接 C++ 标准库
fn cargo_rustc_link_cpp_lib(target: &TargetTriple) {
    // 链接 c++ runtime
    if target.is_linux() {
        println!("cargo:rustc-link-lib=dylib=stdc++");
...
fn cargo_rustc_link_llama_cpp_lib(out: &Path, build: &Path, target: &TargetTriple) {
...
/// 调整 lib 的名称
fn extract_lib_names(out_dir: &Path, target: &TargetTriple) -> Vec<String> {
    let lib_pattern = if target.is_windows() { "*.lib" } else { "*.a" };
    let libs_dir = out_dir.join("lib*");
    let pattern = libs_dir.join(lib_pattern);
    println!("cargo:warning=Extract libs {}", pattern.display());

    let pattern = pattern.to_str().expect("Failed to get lib pattern str");
                    ...
                    && let Some(stem_str) = stem.to_str()
                {
                    ...
                    let lib_name = if stem_str.starts_with("lib") {
                            ...
                            && let Some(parent) = path.parent()
                        {
                            ...
                            rename(&path, &target).expect("Failed to rename lib");
                        }
                        stem_str
                    };
                    lib_names.push(lib_name.to_string());
                } else {
                    panic!("Failed to get lib file stem str, {path:?}")
                }
            }
            Err(e) => panic!("Match failure, error was {e:?}"),
        }
    }
    lib_names
}
...
/// 针对 openmp 进行配置
#[cfg(feature = "openmp")]
fn open_openmp_backend(cmake_config: &mut Config, target: &TargetTriple) {
    // openmp 在安卓上性能提升不明显，安卓平台优先使用 vulkan 编译
    if !target.is_android() {
...
/// 针对 CUDA 进行配置
#[cfg(any(feature = "cuda", feature = "cuda-no-vmm"))]
fn open_cuda_backend(cmake_config: &mut Config) {
...
/// 针对 vulkan 进行配置
#[cfg(feature = "vulkan")]
fn open_vulkan_backend(cmake_config: &mut Config, target: &TargetTriple) {
...
fn make_cmake_config(llama_src: &Path, target: &TargetTriple) -> Config {
    ...
    let parallel_level = env::var("BUILD_PARALLEL_LEVEL")
        .map_or(
            std::thread::available_parallelism()
                .expect("Failed to obtain an estimate of the default amount of parallelism a program should use!")
...
///
/// 指定需要关注的函数和类型
fn make_bindgen(llama_src: &Path) -> Bindings {
...
fn cargo_rerun_if_file_changed(llama_src: &Path) {
    ...
    for entry in entry_iter {
        let entry = entry.expect("Failed to obtain file entry when watching llama.cpp!");
