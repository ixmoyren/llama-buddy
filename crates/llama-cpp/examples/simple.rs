...
fn main() -> Result<(), Whatever> {
    ...
    let tokens = vocab
        ...
        .with_whatever_context(|_| "Failed to tokenize the prompt")?;
    ...
    let mut context = runtime
        ...
        .with_whatever_context(|_| "Failed to create context")?;
    ...
    for token in &tokens {
        let token_str = vocab
            ...
            .with_whatever_context(|_| "Failed to get token string")?;
        // 打印令牌对应的字符串
        print!("{token_str}");
    }
    ...
    // 为提示词准备一个批次
    let mut batch = Batch::get_one(&tokens).with_whatever_context(|_| "Failed to get batch")?;
    // 需要使用一个可变的上下文
    ...
    while pos + batch.n_tokens() < tokens.len() as i32 + token_quantity {
        ...
        context
            .decode(&mut batch)
            .with_whatever_context(|_| "Failed to decode batch")?;
        // 更新当前位置
        pos += batch.n_tokens();
        // 采样下一个令牌
        ...
        let token_str = &vocab
            .token_to_piece(&token, 0, true)
            .with_whatever_context(|_| "Failed to get token str")?;
        print!("{token_str}");
        // 为下一个批次准备一个新的令牌
        batch = Batch::get_one(&[token]).with_whatever_context(|_| "Failed to get a new batch")?;
        decode_num += 1;
    }
    println!();
